...
async function ensureUniqueVersionKey(input: {
  ...
  const rows = await queryRows(
    input.transaction,
    sql`select stable_id from ${sql.identifier(TABLES.figureVersions)} where prototype_id = ${input.prototypeId} and normalized_version_key = ${input.normalizedVersionKey} and deleted_at is null ${excludeCurrent} limit 1`,
  )
  if (rows.length) {
    throw new CatalogDomainError(
