...
const ids = Array.from(
  { length: 12 },
  (_, index) => `INIT-${String(index + 1).padStart(2, "0")}`,
);
const results = [];
const errors = [];
...
if (!argumentsMap["--markers"] || !argumentsMap["--output"]) {
  process.stderr.write(
    "Usage: node verify-init-results.mjs --markers=<directory> --output=<file>\n",
  );
  process.exit(2);
}
...
if (errors.length > 0) {
  process.stderr.write(
    `INIT result validation failed:\n${errors.map((error) => `  - ${error}`).join("\n")}\n`,
  );
  process.exit(1);
}
process.stdout.write(
  `INIT result validation passed: ${summary.counts.pass}/12 pass.\n`,
);
