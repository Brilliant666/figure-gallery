...
test('collect CLI accepts both the documented option and Windows npm positional forwarding without network', () => {
  for (const args of [['--query', '柴郡'], ['柴郡']]) {
    ...
    assert.equal(result.sourceMode, 'hpoi_search_index')
    assert.equal(result.hpoiStatus, 'blocked_by_source')
    assert.equal(result.hpoiRequests, 0)
    assert.equal(result.hpoiDirectHttpRequests, 0)
    assert.equal(result.hpoiDirectBrowserNavigations, 0)
    assert.equal(result.hpoiScrapeRequests, 0)
    assert.equal(result.hpoiApiRequests, 0)
    ...
    assert.match(result.notice, /index|official/i)
  }
})
